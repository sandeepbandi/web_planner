...
class BloodGroup(models.Model):
    ...
    new_ = fields.Char(string = "New Name", compute="new_name_comp")
    new_name = fields.Char(string = "mwe")

    @api.one
    def new_name_comp(self):
        obj = self.env['hr.employee'].search([])
        print obj,"&&&&&&&&&&&&&******************8"
        for i in obj:
            self.new_name = i.name
            print self.new_name,"newnmaejkaskdh"
